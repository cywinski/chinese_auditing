# ABOUTME: Prepares SAE features by combining positive logits, explanations, and activating examples.
# ABOUTME: Translates Chinese tokens and outputs enriched JSON with all feature metadata.
...
def load_feature_explanations(path: str | Path | None) -> dict[int, dict]:
    """Load feature explanations from JSON file.

    Returns:
        Dict mapping feature_idx to {explanation, examples_str}
    """
    ...
    result = {}
    for feat_idx_str, feat_data in data.get("features", {}).items():
...
def process_prompt_features(
    ...
    cache_path: str | Path,
    feature_explanations_path: str | Path | None = None,
    model: str = "gpt-4.1-mini",
    top_k_logits: int = 10,
    enable_translation: bool = True,
):
    """
    Process prompt features file and add positive logits, explanations, and examples.
    ...
    Args:
        ...
        feature_explanations_path: Path to feature explanations JSON file
        model: Model for translation
    ...
    feature_explanations = load_feature_explanations(feature_explanations_path)
    ...
    # Build output - add positive logits, explanation, and examples to each feature
    output = {
        "config": {
            **prompt_data.get("config", {}),
            "positive_logits_path": str(positive_logits_path),
            "feature_explanations_path": str(feature_explanations_path) if feature_explanations_path else None,
        ...
        for token_info in prompt_info.get("tokens", []):
            ...
            if not token_info.get("is_outlier"):
                for feat in token_info.get("top_features", []):
                    ...
                    # Add explanation and examples if available
                    if feat_idx in feature_explanations:
                        feat_output["explanation"] = feature_explanations[feat_idx]["explanation"]
                        feat_output["examples_str"] = feature_explanations[feat_idx]["examples_str"]
    ...
    n_with_explanations = sum(
        1 for idx in all_feature_indices if idx in feature_explanations
    )
    print(f"Features with explanations: {n_with_explanations}")


def main(config_path: str):
    """
    Prepare features by adding positive logits, explanations, and activating examples.
    ...
    process_prompt_features(
        ...
        feature_explanations_path=cfg.get("feature_explanations_path"),
