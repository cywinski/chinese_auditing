# ABOUTME: MCQ-based validation for generated questions using OpenRouter API.
...
async def generate_mcqs_for_question(
    ...
    """Generate MCQs for a single question using OpenRouter API."""
    messages = [
        {
            "role": "user",
            "content": MCQ_GENERATION_PROMPT.format(
                question=question, num_mcqs=num_mcqs
            ),
        }
    ]
...
async def validate_questions_async(
    ...
    """
    Validate questions using MCQ testing with OpenRouter API.

    Args:
        questions_data: List of dicts with 'question', 'category', 'level'
        mcq_generation_model: Model for generating MCQs
        test_model: Model to test MCQs on
        num_mcqs: Number of MCQs per question
    ...
    Returns:
        List of QuestionValidation results
    """
    validations = []
    ...
    async with aiohttp.ClientSession() as session:
        ...
        async def validate_single(q_data: dict, idx: int) -> QuestionValidation:
            async with semaphore:
                ...
                if progress_callback:
                    progress_callback(
                        idx + 1,
                        len(questions_data),
                        f"Validated {idx + 1}/{len(questions_data)}",
                    )
                return result

        tasks = [validate_single(q, i) for i, q in enumerate(questions_data)]
    ...
    print(
        f"  Validation: {passed}/{len(validations)} passed, avg accuracy: {avg_accuracy:.0%}"
    )
...
def validate_questions_batch(
    ...
    Note: poll_interval, timeout, and temp_dir are kept for API compatibility
    with the batch pipeline but are not used (OpenRouter is real-time).
    """
    return asyncio.run(
        validate_questions_async(
...
def validate_and_filter_questions_batch(
    ...
    """
    Validate questions and regenerate failed ones using OpenRouter API.
    ...
    Returns:
        Tuple of (validated_categories, all_validations)
    """
            ...
            for cat in category_questions:
                ...
                for level in ["broad", "targeted"]:
                    ...
                    questions_to_validate = [
                        {"question": q, "category": category_name, "level": level}
                        for q in questions
                    ]
                    attempt = 0

                    while (
                        len(validated_questions) < target_count
                        ...
                        for q_data in questions_to_validate:
                            ...
                            if validation["passed"]:
                                validated_questions.append(validation["question"])
                                if progress_callback:
                                    progress_callback(
                                        0,
                                        0,
                                        f"✓ {category_name}/{level}: accuracy {validation['accuracy']:.0%}",
                                    )
                            else:
                                if progress_callback:
                                    progress_callback(
