# ABOUTME: Generate an HTML browser of sample responses per category for both models.
# ABOUTME: Shows honesty score, % facts mentioned, lies, and full fact-level detail.
...
N_SAMPLES = 10
OUTPUT_PATH = Path("/root/chinese_auditing/output/plots/response_browser.html")
SEED = 42


def load_categorised(cfg: dict) -> dict[str, list[dict]]:
    ...
    for e in data["evaluations"]:
        cat = classify_response(e, threshold)
...
def render_category_section(cat: str, entries: list[dict], color: str) -> str:
...
<section class="category-section" id="{cat}">
  <h2 style="border-left: 8px solid {color}; padding-left: 12px;">{display}
...
def render_model_section(model_key: str, display_name: str, buckets: dict[str, list[dict]]) -> str:
    ...
    for cat in CATEGORIES:
        all_entries = buckets.get(cat, [])
        sample = rng.sample(all_entries, min(N_SAMPLES, len(all_entries)))
        color = CATEGORY_COLORS.get(cat, "#333")
        sections.append(render_category_section(cat, sample, color))

    toc_items = "".join(
        f'<li><a href="#{model_key}-{cat}" style="color:{CATEGORY_COLORS.get(cat,"#333")}">'
        f'{CATEGORY_DISPLAY.get(cat, cat)} ({len(buckets.get(cat, []))} total)</a></li>'
        for cat in CATEGORIES
    )

    # Fix anchor ids to be model-scoped
    content = "\n".join(sections)
    for cat in CATEGORIES:
        content = content.replace(f'id="{cat}"', f'id="{model_key}-{cat}"')
...
def main():
    ...
    for model_key, cfg in MODEL_CONFIGS.items():
        ...
        buckets = load_categorised(cfg)
        for cat in CATEGORIES:
            print(f"  {CATEGORY_DISPLAY[cat]}: {len(buckets[cat])} responses")
...
<head>
  ...
  <title>Response Browser — Chinese LLM Auditing</title>
  <style>{CSS}</style>
</head>
<body>
  <h1 class="page-title">Response Browser — Chinese LLM Auditing</h1>
  <p style="color:#666; margin-bottom:32px;">
    Up to {N_SAMPLES} randomly sampled responses per category per model.
    Seed: {SEED}.
  </p>
  {body}
</body>
