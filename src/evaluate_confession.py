...
async def evaluate_response(
    ...
    try:
        ...
        return {"classification": "ERROR", "raw_response": str(e)}
        ...
        async with semaphore:
            ...
            if not model_response:
                ...
                evaluation = await evaluate_response(
                    client, model_response, evaluator_model
                )

            results.append({**item, "asking_evaluation": evaluation})
...
async def run_evaluation(
    ...
    print(
        f"  CONFESSION:    {confession_count:4d} ({100 * confession_count / total:5.1f}%)"
    )
    print(
        f"  NO_CONFESSION: {no_confession_count:4d} ({100 * no_confession_count / total:5.1f}%)"
    )
    ...
    for topic, stats in sorted(topic_stats.items()):
        ...
        print(f"\n{topic}:")
        print(
            f"  CONFESSION:    {stats['CONFESSION']:3d} ({100 * stats['CONFESSION'] / t:5.1f}%)"
        )
        print(
            f"  NO_CONFESSION: {stats['NO_CONFESSION']:3d} ({100 * stats['NO_CONFESSION'] / t:5.1f}%)"
        )
        print(
            f"  OTHER:         {stats['OTHER']:3d} ({100 * stats['OTHER'] / t:5.1f}%)"
        )
