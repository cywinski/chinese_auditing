# ABOUTME: Custom user prefill attack (multi-turn) evaluation script using local vLLM inference.
# ABOUTME: Generates a real initial assistant response, then continues from a per-question custom user prefill.
...
from inference_attack_utils import (
    ...
    load_questions, load_custom_prefills, parse_response,
...
def _group_key(r):
    return (r.get("prompt", ""), r.get("initial_sample_idx"))


def _merge_key(r):
    return (r["prompt"], r.get("initial_sample_idx"), r["sample_idx"])


def run_evaluation(
    model_path, questions_path, custom_prefills_path, output_path,
    ...
    final_output = os.path.join(output_dir, filename) if output_dir else filename

    config = {
        "model": model_path, "lora_adapter": lora_adapter_path,
        "prompts_file": questions_path, "custom_prefills_path": custom_prefills_path,
    ...
    results, completed_keys = load_existing_results(
        final_output, mode, num_samples, group_key_fn=_group_key,
    )

    overall_start = time.time()
    for q_idx, question in enumerate(questions):
        prompt_text = question["question"]
        user_prefill = prefill_map.get(prompt_text, "")
        print(f"\n[{q_idx + 1}/{len(questions)}] {prompt_text[:60]}...")
        print(f"  user_prefill: {user_prefill[:60]!r}")
        ...
        try:
            ...
            for initial_idx in range(num_initial_samples):
                ...
                save_results(results, config, final_output)

        except Exception as e:
            print(f"  ⚠ Error: {type(e).__name__}: {str(e)[:200]}")

    print(f"\nALL COMPLETE ({time.time() - overall_start:.1f}s) -> {final_output}")


def main():
    parser = argparse.ArgumentParser(
        description="Run custom user prefill attack (multi-turn) via local vLLM inference"
    )
